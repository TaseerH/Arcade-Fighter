...
public class DefaultCharacterSelectionScreen : CharacterSelectionScreen {
	...
	private void Start()
	{
		...
		{
			int charIndex = PlayerPrefs.GetInt("character"); // Character to be selected (0 = first character)
			UFE.SetPlayer(1, UFE.config.characters[charIndex]);
			UFE.StartStoryModeOpeningScreen(0); // Bypass the default fade out value
		}
	}
