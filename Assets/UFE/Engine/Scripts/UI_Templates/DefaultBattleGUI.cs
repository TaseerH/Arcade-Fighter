...
public class DefaultBattleGUI : BattleGUI{
	...
	private void Start()
	{
		...
		UFE.OnRoundEnds += UFE_OnRoundEnds;
    ...
    private void UFE_OnGameBegin(ControlsScript player1, ControlsScript player2, StageOptions stage)
    {
		...
		Debug.Log("Game Begin Health at standard " + UFE.GetPlayer1ControlsScript().myInfo.lifePoints);
	}

    private void UFE_OnRoundEnds(ControlsScript winner, ControlsScript loser)
    {
		UFE.GetPlayer1ControlsScript().currentLifePoints = UFE.GetPlayer1ControlsScript().myInfo.lifePoints;
		Debug.Log("Round End Health at standard");
	}

    private void UFE_OnRoundBegins(int newInt)
    {
		
			Debug.Log("Tried to add too much health");
			currentHealth = UFE.GetPlayer1ControlsScript().myInfo.lifePoints + PlayerPrefs.GetInt("Health");
			UFE.GetPlayer1ControlsScript().currentLifePoints = currentHealth;
			Debug.Log("Round Begin Health Added");
